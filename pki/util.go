...
func ToCertObjectWithCA(componentName, commonName, ouName string, certificate *x509.Certificate, key *rsa.PrivateKey, csrASN1 []byte, caCertPath string) CertificatePKI {
...
func ToCertObject(componentName, commonName, ouName string, certificate *x509.Certificate, key *rsa.PrivateKey, csrASN1 []byte) CertificatePKI {
	return ToCertObjectWithCA(componentName, commonName, ouName, certificate, key, csrASN1, GetCertPath(CACertName))
}
...
func populateCertMap(tmpCerts map[string]CertificatePKI, localConfigPath string, extraHosts []*hosts.Host) map[string]CertificatePKI {
	...
	if certs[EtcdCACertName].Certificate != nil {
		certs[EtcdCACertName] = ToCertObjectWithCA(EtcdCACertName, "", "", tmpCerts[EtcdCACertName].Certificate, tmpCerts[EtcdCACertName].Key, nil, GetCertPath(EtcdCACertName))
	}
	// KubeAPIEtcdClient
	if certs[KubeAPIEtcdClientCertName].Certificate != nil {
		certs[KubeAPIEtcdClientCertName] = ToCertObjectWithCA(KubeAPIEtcdClientCertName, "", "", tmpCerts[KubeAPIEtcdClientCertName].Certificate, tmpCerts[KubeAPIEtcdClientCertName].Key, nil, GetCertPath(EtcdCACertName))
	}
	...
	for _, host := range extraHosts {
		...
		if certs[EtcdCACertName].Certificate != nil {
			certs[etcdName] = ToCertObjectWithCA(etcdName, "", "", etcdCrt, etcdKey, nil, GetCertPath(EtcdCACertName))
		} else {
